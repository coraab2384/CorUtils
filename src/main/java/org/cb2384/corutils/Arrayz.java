...
public class Arrayz {
    ...
    /**
     * Creates an array of generic {@code Object}, but then casts it to the indicated class.
     * This is not safe if the array is ever recast,
     * or exposed in such a way that it could be recast.
     * It is an <i>UNCHECKED</i> array, whose re-casting could generate {@link ArrayStoreException}.
     *
    ...
    private static <I, O> void mapArray(
        ...
        for (int i = start; i < length; i++) {
            toStore[i + resultOffset] = mapper.apply(source[i]);
        }
    }
